    ...
    if req:
        ...
        if ver is not None:
            if _parse_version(ver) < _parse_version(req.lstrip(">=").strip()):
                return False, f"{ver} requires {req}"
    return True, ""
...
def _pip_install(pip_name, upgrade=False):
    ...
    try:
        ...
        return False, "pip install timed out (15 min)"
    except Exception as e:
        return False, str(e)
...
def _bootstrap_deps(progress_cb=None):
    ...
    if not missing:
        if progress_cb: progress_cb("done", "All dependencies present")
        return True, "All dependencies present"

    for imp_name, pip_name, req, why in missing:
        if progress_cb:
            progress_cb("install", f"Installing {pip_name}  ({why})")
        ok, out = _pip_install(pip_name, upgrade=bool(req))
        if not ok:
            if progress_cb:
                progress_cb("error", f"{pip_name} install failed:\n{out[-400:]}")
            return False, f"{pip_name} install failed"
    ...
    if still:
        return False, "Some dependencies are still missing:\n" + "\n".join(still)

    if progress_cb:
        progress_cb("done", "Dependencies installed")
...
def _load_libs():
    ...
    try:
        ...
        sd = None
    logging.info("Third-party libraries loaded")
...
class Capabilities:
    def __init__(self):
        ...
        logging.info("Capabilities: cores=%d ram=%.1fGB ffmpeg=%s tier=%s",
    ...
    def summary(self):
        ff = "yes" if self.has_ffmpeg else "no"
        return (f"{self.cpu_cores} cores / {self.ram_gb:.1f}GB / "
                f"ffmpeg:{ff} / tier:{self.tier}")
...
def pick_font(root):
    try:
        ...
        for f in ["Segoe UI", "Microsoft YaHei UI", "Helvetica Neue",
    ...
    def close(self):
        if self.out:
            try:
                ...
                logging.exception("VideoWriter release failed")
            self.out = None
        logging.info("writer closed: wrote %d frames, dropped %d",
...
class FfmpegBackend(EncoderBackend):
    ...
    def start(self, path, fps, size, options=None):
        ...
        try:
            self.proc = subprocess.Popen(
                ...
                creationflags=flags)
            logging.info("FfmpegBackend started: %s ...", " ".join(cmd[:10]))
        except Exception:
            logging.exception("ffmpeg start failed")
            self.proc = None
    ...
    def write(self, frame, ts):
        ...
        try:
            ...
            logging.exception("ffmpeg write failed")
    ...
    def stop(self):
        if self.proc:
            try:
                ...
                logging.exception("ffmpeg stop failed")
                try:
                    self.proc.kill()
                except Exception:
                    pass
...
def pick_encoder(caps, user_pref="auto", options=None):
    ...
    if user_pref == "ffmpeg":
        if not caps.ffmpeg_path:
            raise RuntimeError("ffmpeg requested but not found")
...
def audio_available():
    ...
    if _parse_version(getattr(sd, "__version__", "0")) < (0, 4, 6):
        return False, f"sounddevice {sd.__version__} is too old, need >=0.4.6"
    try:
        sd.WasapiSettings(loopback=True)
    except Exception as e:
        return False, f"WASAPI loopback unavailable: {e}"
    return True, ""
...
def list_audio_sources():
    ...
    if ok:
        try:
            ...
            logging.exception("Failed to enumerate loopback")
    else:
        logging.warning("System audio unavailable: %s", why)
    ...
    def start(self):
        ...
        if self.source in ("system", "both"):
            if self.system_id is None:
                raise RuntimeError("No system-audio device selected")
    ...
    def stop(self):
        ...
        for s in (self.stream_mic, self.stream_sys):
            try:
                ...
                logging.exception("Audio stream close failed")
...
def merge_av_ffmpeg(ffmpeg_path, video_path, wav_path, out_path=None):
    ...
    try:
        ...
        if r.returncode == 0 and os.path.isfile(out_path):
            return out_path
        logging.error("Merge failed: %s", r.stdout.decode("utf-8", errors="ignore")[-500:])
    except Exception:
        logging.exception("Merge error")
    return None
...
def ffmpeg_export_copy(ffmpeg_path, in_path, out_path, start_sec, dur_sec):
    ...
    try:
        ...
        logging.exception("Stream copy export failed")
        return False
...
class VideoCapturer:
    ...
    def refresh(self):
        try:
            ...
            logging.info("Detected %d monitor(s)", len(self.monitors))
            for i, m in enumerate(self.monitors):
                logging.info("  [%d] %dx%d @(%d,%d)",
                             i, m["width"], m["height"], m["left"], m["top"])
        except Exception:
            logging.exception("mss refresh failed")
            self.monitors = []
    ...
    def monitor_label(self, i):
        if i >= len(self.monitors):
            return ""
        m = self.monitors[i]
        tag = "All monitors" if i == 0 else f"Monitor {i}"
...
class DependencySplash:
    ...
    def show(self):
        root = tk.Tk()
        root.title(f"{APP_NAME} - Preparing")
        root.geometry("440x200")
        root.resizable(False, False)
        root.configure(bg="#1e1e1e")

        tk.Label(root, text="Checking environment...", bg="#1e1e1e", fg="#e0e0e0",
        ...
        def cb(stage, msg):
            def upd():
                ...
                if stage == "done":
                    ...
                    bar.stop()
                    messagebox.showerror("Install failed", msg, parent=root)
        ...
        def worker():
            ...
            if not ok:
                ...
                try:
                    root.after(0, lambda: messagebox.showerror("Failed", msg, parent=root))
                except Exception:
                    pass
                try:
...
class RegionSelector:
    ...
    def show(self):
        ...
        cv.create_text(v["width"] // 2, 30,
                       text="Drag to select region  |  ESC to cancel  |  Enter to confirm  (multi-monitor supported)",
...
class RecorderPage:
    ...
    def _build(self):
        ...
        w3, i3 = self._section("Recording settings")
        ...
        tk.Label(i3, text=f"(recommended {self.app.caps.recommend_fps()})",
                 bg=c["card"], fg=c["muted"], font=f["small"]
                 ).grid(row=0, column=2, sticky="w", padx=4)

        tk.Label(i3, text="Encoder:", bg=c["card"], fg=c["fg"], font=f["body"]
        ...
        ff_txt = "ffmpeg available" if self.app.caps.has_ffmpeg else "no ffmpeg (using OpenCV)"
        tk.Label(i3, text=ff_txt, bg=c["card"], fg=c["muted"], font=f["small"]
                 ).grid(row=1, column=2, sticky="w", padx=4)

        tk.Label(i3, text="Quality CRF:", bg=c["card"], fg=c["fg"], font=f["body"]
        ...
        tk.Label(i3, text="(lower = better, 18~28 recommended, ffmpeg only)",
                 bg=c["card"], fg=c["muted"], font=f["small"]
                 ).grid(row=2, column=2, sticky="w", padx=4)

        tk.Label(i3, text="Format:", bg=c["card"], fg=c["fg"], font=f["body"]
        ...
        tk.Label(i3, text="Output:", bg=c["card"], fg=c["fg"], font=f["body"]
        ...
        ctrl.pack(pady=10)
        self.start_btn = tk.Button(ctrl, text="● Start recording",
        ...
        w4, i4 = self._section("Status")
        w4.pack(fill="both", expand=True, pady=(6, 4))
        self.status_label = tk.Label(i4, text="Ready", font=f["body"],
    ...
    def _refresh_audio(self):
        ...
        parts = []
        if self.mic_source:
            parts.append(f"Mic: {self.mic_source['name']}")
        if self.system_source:
            parts.append(f"System: {self.system_source['name']}")
        if not parts:
            parts.append(f"No audio devices ({msg})")
...
class EditorPage:
    ...
    def _build(self):
        ...
        proll.pack(pady=(2, 6))
        self.play_btn = tk.Button(proll, text="▶ Play", command=self.play,
                                  bg=c["success"], fg="#fff", relief="flat",
                                  padx=14, pady=4, cursor="hand2", font=f["small"])
        self.play_btn.pack(side="left", padx=4)
        self.pause_btn = tk.Button(proll, text="⏸ Pause", command=self.pause,
                                   bg=c["warning"], fg="#fff", relief="flat",
                                   padx=14, pady=4, cursor="hand2", font=f["small"])
        self.pause_btn.pack(side="left", padx=4)

        w4, i4 = self._section("Clip")
        w4.pack(fill="x", pady=(6, 4))
        crow = tk.Frame(i4, bg=c["card"])
        crow.pack(pady=4)
        tk.Button(crow, text="Set start", command=self.set_start,
                  bg=c["accent"], fg="#fff", relief="flat",
                  padx=12, pady=4, cursor="hand2", font=f["small"]
                  ).pack(side="left", padx=4)
        tk.Button(crow, text="Set end", command=self.set_end,
                  bg=c["accent"], fg="#fff", relief="flat",
                  padx=12, pady=4, cursor="hand2", font=f["small"]
                  ).pack(side="left", padx=4)
        self.export_btn = tk.Button(crow, text="✂ Export clip", command=self.export,
                                    bg=c["danger"], fg="#fff", relief="flat",
                                    padx=12, pady=4, cursor="hand2", font=f["small"])
    ...
    def export(self):
        ...
        def worker():
            try:
                ...
                cap.release()
                self.app.root.after(0, lambda: messagebox.showinfo(
                    "Done", f"Exported (re-encoded)\n{writer.path}"))
            except Exception as e:
                logging.exception("Export failed")
                self.app.root.after(0, lambda: messagebox.showerror("Failed", str(e)))
            finally:
                self.is_exporting = False
...
class SettingsDialog:
    ...
    def show(self):
        ...
        tk.Label(self.win, text="System", font=f["body"],
        ...
        tk.Label(self.win, text="ffmpeg", font=f["body"],
                 bg=c["bg"], fg=c["accent"]).pack(anchor="w", padx=15, pady=(15, 4))
        ff = self.app.caps.ffmpeg_path or "Not found (OpenCV backend will be used; export will re-encode)"
        tk.Label(self.win, text=ff, font=f["small"], bg=c["bg"], fg=c["muted"],
                 wraplength=440, justify="left").pack(anchor="w", padx=15)

        tk.Label(self.win, text="Log file", font=f["body"],
                 bg=c["bg"], fg=c["accent"]).pack(anchor="w", padx=15, pady=(15, 4))
        ...
        tk.Button(btn_row, text="Open log", command=open_log,
                  bg=c["card"], fg=c["fg"], relief="flat",
...
class ScreenRecorderApp:
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="⚙ Settings", command=lambda: SettingsDialog(self).show(),
                  bg=c["card"], fg=c["fg"], relief="flat",
                  padx=12, pady=4, cursor="hand2", font=f["small"]
                  ).pack(side="right")
        ...
        self.tabs = {}
        for name, label in [("recorder", "Record"), ("editor", "Edit")]:
    ...
    def start_recording(self):
        ...
        try:
            ...
            messagebox.showerror("Encoder selection failed", str(e))
            return
        ...
        if mode != "none":
            try:
                ...
                if mode == "system" and sys_id is None:
                    raise RuntimeError("No system-audio device available")
                if mode == "both" and (mic_id is None or sys_id is None):
                    raise RuntimeError("Mixing requires both mic and system audio")
                self.audio_recorder = AudioRecorder(
                    source=mode, mic_id=mic_id, system_id=sys_id)
                self.audio_recorder.start()
                logging.info("Audio started: %s", mode)
            except Exception as e:
                logging.exception("Failed to start audio")
                messagebox.showwarning("Audio unavailable",
                    f"Video will be recorded without audio.\n\n{e}")
                self.audio_recorder = None

        self.recording = True
        self.paused = False
        rp.start_btn.config(state="disabled")
        rp.pause_btn.config(state="normal", text="⏸ Pause")
        rp.stop_btn.config(state="normal")
        rp.status_label.config(text="Recording...", fg=self.theme.colors["success"])
        ...
        while self.recording:
            ...
            if frame is None:
                ...
                if self.capture_error_count >= self.max_capture_errors:
                    logging.error("Capture failed %d times, stopping", self.max_capture_errors)
    ...
    def _on_recording_finished(self):
            ...
            try:
                ...
                if data is not None and len(data) > 0:
                    ...
                    if self.audio_recorder.save_wav(wav_path, data):
                        ...
                        logging.info("Audio saved: %s", wav_path)
            except Exception:
        ...
        if self.audio_wav_path and self.caps.ffmpeg_path:
            try:
                ...
                if merged_path:
                    logging.info("Merged: %s", merged_path)
            except Exception:
        ...
        rp.pause_btn.config(state="disabled", text="⏸ Pause")
        rp.stop_btn.config(state="disabled")
        rp.status_label.config(text="Recording complete", fg=self.theme.colors["fg"])

        msg = f"Video: {self.output_path}"
        if self.audio_wav_path:
            msg += f"\nAudio: {self.audio_wav_path}"
        if merged_path:
            msg += f"\nMerged: {merged_path}"
    ...
    def pause_recording(self):
        ...
        if self.paused:
            ...
            rp.pause_btn.config(text="▶ Resume")
            rp.status_label.config(text="Paused", fg=self.theme.colors["warning"])
            if self.audio_recorder:
            ...
            if self.pause_started_at:
                ...
                self.pause_started_at = None
            rp.pause_btn.config(text="⏸ Pause")
            rp.status_label.config(text="Recording...", fg=self.theme.colors["success"])
            if self.audio_recorder:
    ...
    def stop_recording(self):
        if self.recording:
            ...
            self.recorder_page.status_label.config(
                text="Stopping...", fg=self.theme.colors["warning"])
...
def main():
    ...
    if not splash.show():
        try:
            ...
            messagebox.showerror(
                "Unable to start",
                "Dependencies not ready. Please run:\n\n"
    ...
    try:
        ...
        logging.exception("Main program error")
        try:
            import tkinter as tk
            from tkinter import messagebox
            r = tk.Tk(); r.withdraw()
            messagebox.showerror("Startup failed", f"{e}\n\nLog: {LOG_FILE}")
            r.destroy()
        except Exception:
            pass
