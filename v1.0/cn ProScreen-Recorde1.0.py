...
class ScreenRecorderApp:
    def __init__(self):
        try:
            ...
            self.fonts = {
                'title': tkfont.Font(family='Microsoft YaHei', size=18, weight='bold'),
                'heading': tkfont.Font(family='Microsoft YaHei', size=12, weight='bold'),
                'body': tkfont.Font(family='Microsoft YaHei', size=10),
                'small': tkfont.Font(family='Microsoft YaHei', size=9),
                'button': tkfont.Font(family='Microsoft YaHei', size=10, weight='bold'),
                'large_button': tkfont.Font(family='Microsoft YaHei', size=11, weight='bold'),
                'timer': tkfont.Font(family='Microsoft YaHei', size=28, weight='bold'),
            }
    ...
    def setup_ui(self):
        ...
        self.tabs = {}
        self.create_tab_button("recorder", "🎥 录制")
        self.create_tab_button("editor", "✂️ 剪辑")
    ...
    def create_recorder_page(self):
        ...
        # 模式卡片
        mode_card, mode_inner = self.create_card(self.recorder_page, "录制模式")
        ...
        tk.Radiobutton(mode_inner, text="🖥 全屏录制", variable=self.mode_var,
        ...
        tk.Radiobutton(mode_inner, text="🎯 自定义区域", variable=self.mode_var,
                      value='region', bg=self.colors['card'], fg=self.colors['fg'],
                      selectcolor=self.colors['input_bg'], font=self.fonts['body'],
                      activebackground=self.colors['card'], cursor='hand2'
                      ).grid(row=0, column=1, padx=10, pady=10, sticky='w')
        
        self.select_region_btn = tk.Button(mode_inner, text="选择区域",
        ...
        # 设置卡片
        set_card, set_inner = self.create_card(self.recorder_page, "录制设置")
        set_card.pack(fill='x', padx=15, pady=8)
        
        set_inner.grid_columnconfigure(1, weight=1)
        
        tk.Label(set_inner, text="帧率 (FPS):", bg=self.colors['card'],
        ...
        tk.Label(set_inner, text="格式:", bg=self.colors['card'],
                fg=self.colors['fg'], font=self.fonts['body']
        ...
        tk.Label(set_inner, text="输出路径:", bg=self.colors['card'],
                fg=self.colors['fg'], font=self.fonts['body']
        ...
        tk.Button(path_frame, text="浏览", command=self.browse_output,
                 bg=self.colors['button'], fg=self.colors['button_text'],
        ...
        self.start_btn = tk.Button(control_frame, text="⏺ 开始录制",
        ...
        # 状态卡片
        status_card, status_inner = self.create_card(self.recorder_page, "状态")
        status_card.pack(fill='both', expand=True, padx=15, pady=(8, 15))
        
        self.status_label = tk.Label(status_inner, text="就绪", font=self.fonts['heading'],
    ...
    def create_editor_page(self):
        ...
        # 文件卡片
        file_card, file_inner = self.create_card(self.editor_page, "视频文件")
        ...
        tk.Button(file_inner, text="加载视频", command=self.load_video,
                 bg=self.colors['accent'], fg=self.colors['button_text'],
                 relief=tk.FLAT, padx=12, pady=4, cursor='hand2',
                 font=self.fonts['button']).grid(row=0, column=1)
        
        # 预览卡片
        prev_card, prev_inner = self.create_card(self.editor_page, "预览")
        prev_card.pack(fill='both', expand=True, padx=15, pady=8)
        
        self.preview_label = tk.Label(prev_inner, text="加载视频以预览",
                                      bg=self.colors['input_bg'], fg=self.colors['muted'],
                                      font=self.fonts['body'])
        self.preview_label.pack(fill='both', expand=True)
        
        # 时间轴卡片
        time_card, time_inner = self.create_card(self.editor_page, "时间轴")
        ...
        self.play_btn = tk.Button(play_controls, text="▶ 播放", command=self.play_video,
                                  bg=self.colors['success'], fg=self.colors['button_text'],
                                  relief=tk.FLAT, padx=15, pady=5, cursor='hand2',
                                  font=self.fonts['button'])
        self.play_btn.pack(side='left', padx=5)
        
        self.pause_video_btn = tk.Button(play_controls, text="⏸ 暂停", command=self.pause_video,
        ...
        # 剪辑卡片
        clip_card, clip_inner = self.create_card(self.editor_page, "剪辑控制")
        clip_card.pack(fill='x', padx=15, pady=(8, 15))
        
        clip_controls = tk.Frame(clip_inner, bg=self.colors['card'])
        clip_controls.pack(pady=10)
        
        self.set_start_btn = tk.Button(clip_controls, text="设置起点", command=self.set_clip_start,
                                       bg=self.colors['accent'], fg=self.colors['button_text'],
                                       relief=tk.FLAT, padx=15, pady=5, cursor='hand2',
                                       font=self.fonts['button'])
        self.set_start_btn.pack(side='left', padx=5)
        
        self.set_end_btn = tk.Button(clip_controls, text="设置终点", command=self.set_clip_end,
                                     bg=self.colors['accent'], fg=self.colors['button_text'],
                                     relief=tk.FLAT, padx=15, pady=5, cursor='hand2',
                                     font=self.fonts['button'])
        self.set_end_btn.pack(side='left', padx=5)
        
        self.export_clip_btn = tk.Button(clip_controls, text="✂ 导出片段", command=self.export_clip,
    ...
    def browse_output(self):
        ...
        filename = filedialog.asksaveasfilename(
            ...
            filetypes=filetypes,
            initialfile=f"录屏_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{self.codec_var.get()}"
        )
        if filename:
    ...
    def select_region(self):
        ...
        self.selection_canvas.create_text(
            ...
            text="拖拽选择录制区域\nESC取消，Enter确认",
            fill="white", font=("Microsoft YaHei", 16, "bold"), justify='center'
        )
    ...
    def on_selection_end(self, event):
        if self.selection_start:
            ...
            self.selection_canvas.create_text(
                ...
                text=f"区域: {self.screen_region[2]}x{self.screen_region[3]}\n按Enter确认",
                fill="#3fb950", font=("Microsoft YaHei", 14, "bold"), justify='center'
            )
    ...
    def start_recording(self):
        if self.recording:
            messagebox.showwarning("警告", "正在录制中")
            return
        if not self.path_var.get():
            ext = self.codec_var.get()
            self.path_var.set(os.path.join(
                os.path.expanduser("~"), "Desktop",
                f"录屏_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{ext}"
            ))
        ...
        self.status_label.config(text="正在录制...", fg=self.colors['success'])
            ...
            with mss.mss() as sct:
                ...
                if not monitors or len(monitors) < 1:
                    self.root.after(0, lambda: self.status_label.config(
                        text="无法获取屏幕", fg=self.colors['danger']))
                    return
    ...
    def write_frames(self):
        ...
        if not out.isOpened():
            self.root.after(0, lambda: messagebox.showerror("错误", "无法创建视频文件"))
            return
    ...
    def on_recording_finished(self):
        ...
        self.status_label.config(text="录制完成", fg=self.colors['fg'])
        self.info_label.config(text=f"已保存: {self.output_path}")
    ...
    def stop_recording(self):
        if self.recording:
            self.recording = False
            self.status_label.config(text="正在停止...", fg=self.colors['warning'])
    ...
    def export_clip(self):
        if self.is_exporting:
            messagebox.showwarning("警告", "正在导出中")
            return
        if not self.video_capture:
            messagebox.showwarning("警告", "请先加载视频")
            return
        if self.clip_start >= self.clip_end:
            messagebox.showwarning("警告", "起点必须早于终点")
            return
        if self.recording:
        ...
        output_path = filedialog.asksaveasfilename(
            ...
            filetypes=[("MP4 文件", "*.mp4"), ("AVI 文件", "*.avi"), ("所有文件", "*.*")],
            initialfile=f"片段_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
        )
        if not output_path:
            return
    ...
    def on_export_finished(self, win, path):
        win.destroy()
        messagebox.showinfo("完成", f"片段导出成功!\n{path}")
    ...
    def open_settings(self):
        ...
        card, inner = self.create_card(win, "字体设置")
        card.pack(fill='x', padx=15, pady=15)
        
        inner.grid_columnconfigure(1, weight=1)
        
        tk.Label(inner, text="字体:", bg=self.colors['card'],
                fg=self.colors['fg'], font=self.fonts['body']
                ).grid(row=0, column=0, sticky='w', padx=5, pady=5)
        
        family_var = tk.StringVar(value='Microsoft YaHei')
        ttk.Combobox(inner, textvariable=family_var,
                    values=['Microsoft YaHei', 'SimHei', 'KaiTi', 'Arial', 'Consolas'],
                    state='readonly').grid(row=0, column=1, sticky='ew', padx=5, pady=5)
        
        tk.Label(inner, text="标题大小:", bg=self.colors['card'],
                fg=self.colors['fg'], font=self.fonts['body']
        ...
        tk.Label(inner, text="正文字号:", bg=self.colors['card'],
                fg=self.colors['fg'], font=self.fonts['body']
        ...
        tk.Button(win, text="保存", command=save, bg=self.colors['success'],
